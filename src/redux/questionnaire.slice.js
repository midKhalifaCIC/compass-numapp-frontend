...
const fetchQuestionnaire = createAsyncThunk(
  "questionnaire/FETCH",
  async ({ questionnaireID, subjectId, startDate, dueDate }, thunkApi) => {
    try {
      const response = await loggedInClient.getBaseQuestionnaire(
        questionnaireID,
        `Bearer: ${subjectId}`
      );
      // to create the metada object the response is copied and the questions (i.e.the top-level items) are removed.
